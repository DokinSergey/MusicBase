-- Выгрузка данных плейлиста в таблицу  Dokin_Playlist  вер.2025-08-02 
...
Insert Into CurPlaylist(Id_DP, Artist, Locale, Title, ArtSort,NewTrack, AllCount, EnRuCount, ArtCount)
Select tracknumber, artist, locale, title, IFNULL(PERFORMER, Artist),0, Count(*) Over(), Count(*) over(partition by Locale), Count(*) over(partition by IFNULL(PERFORMER, Artist))
